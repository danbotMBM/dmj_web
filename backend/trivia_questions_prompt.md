...
Before writing any new questions, always consult `used_answers.md` (generated by `go run ./validate_trivia generate-index trivia_questions.json`). This file lists every answer already used, formatted as `Category: Display Answer`.

**"Same answer" means the same concept, not just the same string.** For example, these are all the same answer and must not be reused:
- "The Nile" / "Nile River" / "River Nile"
- "Einstein" / "Albert Einstein"
- "Great Wall" / "Great Wall of China"

If an answer concept appears anywhere in `used_answers.md`, pick a different answer entirely — don't just rephrase the question.
